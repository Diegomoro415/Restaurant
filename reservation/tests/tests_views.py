...
class ReservationViewsTest(TestCase):
    ...
    def test_reservation_view_POST(self):
        ...
        self.assertTrue(Reservation.objects.filter(phone='9876543210').exists())

    def test_reservation_options_view(self):
        self.client.login(username='testuser', password='testpassword')  # Authenticate the client
    ...
    def test_cancel_reservation(self):
        self.client.login(username='testuser', password='testpassword')
        response = self.client.post(reverse('reservation:cancel_reservation', kwargs={'reservation_id': self.reservation.id}))
        self.assertEqual(response.status_code, 302)
    ...
    def test_reservation_detail_view(self):
        self.client.login(username='testuser', password='testpassword')
        response = self.client.get(reverse('reservation:reservation_detail', args=[self.reservation.id]))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'Reservation/reservation_detail.html')
